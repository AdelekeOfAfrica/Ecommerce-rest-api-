...
class AuditorController extends Controller
{
    public function register(Request $request){
        ...
        #check if user uploads an image 
            if ($request->hasFile('profile_image')) {
                ...
                #save user records
                if (Auditor::create(array_merge($validator->validated(), ['profile_image'=>$profile_image]))) {
                        return response()->json([
            ...
            if(Auditor::create(array_merge($validator->validated(),
    ...
    public function login (Request $request) {
        ...
        if (!Auth::guard('auditor')->attempt($request->only(['email','password']))) { //here we are usig the admin guard for authentication
            return response()->json(['error' => 'invalid email or password'], 401);
        }

        #create token for the user 
        $admin = Auditor::where('email', $request->email)->first();
